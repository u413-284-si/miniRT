...
/*   Updated: 2024/01/17 00:46:13 by sqiu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	ft_co_calc_pot_hits(t_cone co, t_ray ray, float potential_hits[3])
{
	...
	cos_2_angle = pow(cos(co.angle), 2);
	eq.a = pow(dot_product_ray_axis, 2) - cos_2_angle;
	eq.b = 2.0 * (dot_product_ray_axis * dot_product_apex_axis - \
		ft_vec3_dot(ray.direction, ray_apex) * cos_2_angle);
	eq.c = pow(dot_product_apex_axis, 2) - ft_vec3_dot(ray_apex, ray_apex) \
		* cos_2_angle;
	if (!ft_solve(&eq))
		return (false);
